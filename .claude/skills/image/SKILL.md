...
name: image
description: >
  Gera imagens cinematográficas de alto nível pensando como Diretor de
  Fotografia. O usuário NÃO escolhe câmera, lente, luz ou mood — o sistema
  decide tudo e infere o look. Destino de render obrigatório: Higgsfield CLI
  com Nano Banana 2 (`nano_banana_2`) via `Human Images/scripts/render_image.py`.
  Use quando o usuário pedir uma imagem, foto, render, still, capa, key art,
  ou usar `/image`.
---
...
Na prática, sempre invoque o render pelo wrapper `Human Images/scripts/render_image.py`, que monta esse comando, faz upload da referência, faz polling do job e baixa a imagem para o `output-dir`.
